...
class BinaryCallback(tf.keras.callbacks.Callback):
    ...
    def on_train_end(self, logs=None):
        if self.plot:
            ...
            ax_kwargs.plot(x, [np.mean(self.H_L_n_s0_list[i]) for i in range(len(self.H_L_n_list))],
                           label=r'$H(L_n|S_0)$', marker='.', ms=5, color=colors_points[0], linestyle="None")
            ax_kwargs.plot(x, [np.mean(self.H_L_n_s1_list[i]) for i in range(len(self.H_L_n_list))],
...
class BaseModel(BaseEstimator):
    ...
    def _get_quantized_activation(self, activation_str, bits):
    ...
    def _get_hidden_qlayer(
        ...
        last_activation = self._get_quantized_activation(last_activation, bits)
        if not conv:
            nn = QDense(
                hidden_layer[0],
                kernel_quantizer=quantized_bits(bits),
        ...
        nn = QActivation(activation, activity_regularizer=kernel_regularizer, name="act_{}_0".format(name))(nn)
        if drop_out > 0:
            nn = tf.keras.layers.Dropout(drop_out)(nn)
        for i in range(1, len(hidden_layer)):
            if not conv:
                ...
                nn = tf.keras.layers.MaxPooling2D()(nn)
            if last_activation != "" and i == len(hidden_layer) - 1:
                nn = QActivation(last_activation, activity_regularizer=kernel_regularizer,
                                 name="act_{}_{}".format(name, i))(nn)
            else:
                nn = QActivation(activation, activity_regularizer=kernel_regularizer)(nn)
            if drop_out > 0 and (i < (len(hidden_layer) - 1) or extra_layer):
                nn = tf.keras.layers.Dropout(drop_out)(nn)
        if conv:
            nn = tf.keras.layers.Flatten(name='flatten')(nn)
            nn = self._get_qdense(
        ...
        if last_activation != "" and extra_layer:
            nn = tf.keras.layers.Dropout(drop_out)(nn)
            nn = QDense(
                units=1,
                name="nn_last_{}".format(name),
                kernel_quantizer=quantized_bits(bits),
                kernel_regularizer=kernel_regularizer,
                bias_quantizer=quantized_bits(bits)
            )(nn)
            last_q_activation = self._get_quantized_activation(last_activation, bits)
            nn = QActivation(last_q_activation, activity_regularizer=kernel_regularizer)(nn)
            nn = tf.keras.layers.Activation(last_activation, name="nn_out_{}".format(name))(nn)
        if build_bias:
            hidden_part = nn
        else:
            hidden_part = tf.keras.models.Model(input_layer, nn, name=name)
        if self.verbose > 1 and not build_bias:
            hidden_part.summary()
        return hidden_part
